...
async def dependencies() -> dict:
    ...
    return {
        ...
        "ffmpeg": {
            "path": str(ff_path),
            "exists": ff_path.exists() and ffprobe_path.exists(),
            "ffmpeg_exists": ff_path.exists(),
...
async def yt_dlp_update() -> dict:
    ...
    if state_module.app_state is None:
        raise HTTPException(status_code=503, detail="App state not initialized")
    async with state_module.app_state.yt_dlp_install_lock:
        try:
...
async def ffmpeg_update() -> dict:
    ...
    if state_module.app_state is None:
        raise HTTPException(status_code=503, detail="App state not initialized")
    async with state_module.app_state.ffmpeg_install_lock:
        try:
