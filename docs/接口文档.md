...
说明：取消任务（仅未结束任务可取消）。后端会尽力终止对应的下载进程（Windows 下会终止进程树），并将任务状态置为 `canceled`。
...
说明：重试失败任务（仅 `failed` 状态允许重试，会创建一个新任务 ID 重新入队）。
...
2. `ffmpeg.exists`：综合可用性（要求 `ffmpeg` 与 `ffprobe` 同时存在）
3. `ffmpeg.ffmpeg_exists`：仅表示 `ffmpeg` 可执行文件是否存在
4. `ffmpeg.ffprobe_exists`：仅表示 `ffprobe` 可执行文件是否存在
